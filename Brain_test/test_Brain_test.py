...
class Test_api(unittest.TestCase):
    ...
    def test_totalscore(self, data):
        ...
        #123
        print("总分是%s"%data["总分"],"库里总分是%s"%float(str(stulist[0][10])))
        self.assertTrue(abs(data["总分"] - float(str(stulist[0][10])))<0.01)
    ...
    def test_Personal_performance(self, data):
         ...
         print("个人表现是%s"%data["个人表现=学生互评+系统评分+组内互评"],"库里个人表现是%s"%float(str(stulist[0][9])))
         self.assertTrue(abs(data["个人表现=学生互评+系统评分+组内互评"] - float(str(stulist[0][9])))<0.01)
    #
    @ddt.data(*testdata)
    ...
    def test_Brainstorming(self, data):
         ...
         sql = "select * from dbo.AFCS_StudentScore a join dbo.AFCS_Experiment b on a.ExperimentID = b.ExperimentID where ExperimentName = '"+experimentname+"' and StudentID = '"+stuid+"'"
         stulist= self.A.mssql_getrows(sql)
         print("头脑风暴是%s"%data["头脑风暴=头脑风暴+诊断总结"],"库里头脑风暴是%s"%float(str(stulist[0][5])))
         self.assertTrue(abs(data["头脑风暴=头脑风暴+诊断总结"] - float(str(stulist[0][5])))<0.01)
    ...
    def test_Group_performance(self, data):
         ...
         print("小组表现是%s"%data["小组表现=小组互评+教师评分"],"库里小组表现是%s"%float(str(stulist[0][14])))
         self.assertTrue(abs(data["小组表现=小组互评+教师评分"] - float(str(stulist[0][14])))<0.01)
    #
    #
    #
    #
    ...
    def test_Teacher_ratings_of_thinking_questions(self, data):
         ...
         # print("思考题系教师评分是%s"%data["思考题教师评分"],"库里思考题系教师评分是%s"%float(str(stulist[0][6])))
         # self.assertTrue(abs(data["思考题教师评分"] - float(str(stulist[0][6])))<0.01)
         print("思考题系教师评分是%s"%data["思考题教师评分"],"库里思考题系教师评分是%s"%float(str(stulist[0][13])))
         self.assertTrue(abs(data["思考题教师评分"] - float(str(stulist[0][13])))<0.01)
